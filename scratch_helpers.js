...
const WALLED = t => !["parking", "balcony", "stairs"].includes(t);
...
function wallPlan() {
  const key = JSON.stringify(state.rooms.map(r => [r.id, r.type, r.floor, r.x, r.y, r.w, r.h]));
  if (wallPlanCache.key === key) return wallPlanCache.segs;

  const rooms = state.rooms, segs = [];
...
const ext = id => hasItem("exterior", id);
...
function exterior3d(cx, cz, showRoof) {
  const P = state.plot, t = measure(), top = t.floors - 1;
  const mat = c => new THREE.MeshLambertMaterial({ color:c });
  const topRooms = state.rooms.filter(r => r.floor === top && !UNCONDITIONED.has(r.type));
  const g = new THREE.Group();
  ...
  const park = state.rooms.find(r => r.type === "parking" && r.floor === 0);
  if (ext("x-porch") && park) {
  ...
  if (ext("x-tank")) {
    const tank = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 2.2, 4, 16),
                                mat(0x2a2a2e));
    const top = measure().floors * FLOOR_H;
...
function build3d() {
  ...
  while (shell.children.length) {
    ...
    c.traverse?.(n => {
      ...
      (Array.isArray(n.material) ? n.material : [n.material]).forEach(m => {
        if (m && m !== GLASS && m !== FRAME && m !== DOORLEAF && m !== MAINDOOR && m !== CHAJJA) m.dispose?.();
      });
    });
  }
  colliders = []; stairZones = []; spinners = [];
  ...
  const showRoof = $("showRoof").checked || walking;
  ...
  for (const r of state.rooms) {
    ...
    mats.set(r.id, {
      wall: bad.has(r.id) ? BAD : r.id === state.selected ? SEL
  ...
  for (const r of state.rooms) {
    ...
    if (r.type === "stairs") buildStairs(g, r, baseY + .4, cx, cz);
      ...
      const touches = side => state.rooms.some(q => q !== r && q.floor === r.floor && WALLED(q.type) && (
